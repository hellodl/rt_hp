...
void CPMDataTransformer::swapLeftRight(Joints& j) {
  cout<<" >>> Using method swapLeftRight. "<<endl;
  if(np == 56){
...
void CPMDataTransformer::TransformMetaJoints(MetaData& meta) {
  ...
  cout << "---------------------------------------" << endl;
  cout << "Entering method TransformMetaJoints ..." << endl;
...
void CPMDataTransformer::generateLabelMap(double* transformed_label, Mat& img_aug, MetaData meta, int mod) {
  ...
  int mode = mod; // TO DO: make this as a parameter

  cout << "------------------------------------" << endl;
  cout << "Entering method generateLabelMap ..." << endl;

  for(int i=0; i<np; i++){
      cout<< "joint idx: " << i << endl;
      cout<< "joint x: " << meta.joint_self.joints[i].x << endl;
      cout<< "joint y: " << meta.joint_self.joints[i].y << endl;
    }
  ...
  if (np == 56){
    ...
    for(int i=0;i<19;i++){
      ...
      if(jo.isVisible[mid_1[i]-1]<=1 && jo.isVisible[mid_2[i]-1]<=1){
        //putVecPeaks
        // notice the (np + 1 + 2*i) and (np + 2 + 2*i)
  ...
  cout << "Exiting method generateLabelMap ...." << endl;
}
...
void CPMDataTransformer::ReadMetaData(MetaData& meta, const uchar *data, size_t offset3, size_t offset1) { //very specific to genLMDB.py
  cout << "---------------------------------------" << endl;
  cout << "Entering method ReadMetaData ..." << endl;
  // ------------------- Dataset name ----------------------
  ...
  cout << "Exiting method ReadMetaData ..." << endl;
}
...
void CPMDataTransformer::Transform_nv(const uchar *data, const int datum_channels, const int datum_height, const int datum_width, uchar* transformed_data, double* transformed_label) {
  ...
  generateLabelMap(transformed_label, img_aug, meta, mode);
