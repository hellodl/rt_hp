...
void CPMDataTransformer::TransformMetaJoints(MetaData& meta) {
  ...
  cout<< "---------------------------------"<< endl;
  cout<< "meta.joint_others size:"<< meta.joint_others.size() << endl;
...
float CPMDataTransformer::augmentation_scale(Mat& img_src, Mat& img_temp, Mat& mask_miss, Mat& mask_all, MetaData& meta, int mode) {
  ...
  cout<<"In augmentation_scale, dice is: " << dice << endl;
...
void CPMDataTransformer::Transform_nv(const uchar *data, const int datum_channels, const int datum_height, const int datum_width, uchar* transformed_data, double* transformed_label) {
  ...
  const int mode = datum_channels;  //related to datum.channels();
  cout << "The transform mode(datum_channels) is :" << datum_channels << endl;
  int crop_x = param_.crop_size_x;
  int crop_y = param_.crop_size_y;

  //before any transformation, get the image from datum
  cout << "The datum_height is :" << datum_height << endl;
  cout << "The datum_width is :" << datum_width << endl;
  cout<< "---------------------------------"<< endl;
  ...
  //color, contract
  cout << "param_.do_clahe: "<< param_.do_clahe << endl;
  if(param_.do_clahe)
    clahe(img, clahe_tileSize, clahe_clipLimit);

  cout << "param_gray: " << param_.gray << endl;
  if(param_.gray == 1){
  ...
  cout << "param_transform_body_joint: " << param_.transform_body_joint << endl;
  ...
  cout << "img -(scale)-> img_temp -(rotate)-> img_temp2 -(croppad)-> img_temp3 -(flip)-> img_aug" << endl;
  as.scale = augmentation_scale(img, img_temp, mask_miss, mask_all, meta, mode);
  cout << "After scale, img_temp's rows is " << img_temp.rows << endl;
  cout << "After scale, img_temp's cols is " << img_temp.cols << endl;
  ...
  cout << "After rotate, img_temp2's rows is " << img_temp2.rows << endl;
  cout << "After rotate, img_temp2's cols is " << img_temp2.cols << endl;
  as.crop = augmentation_croppad(img_temp2, img_aug, mask_miss, mask_miss_aug, mask_all, mask_all_aug, meta, mode);
  //if(0 && param_.visualize())
  //  visualize(img_temp3, meta, as);
  cout << "After flip, img_temp3's rows is " << img_temp3.rows << endl;
  cout << "After flip, img_temp3's cols is " << img_temp3.cols << endl;
  ...
  cout << "After aug, img's rows is " << img_aug.rows << endl;
  cout << "After aug, img's cols is " << img_aug.cols << endl;
  int rezX = img_aug.cols;
  int rezY = img_aug.rows;
  int grid_x = rezX / stride;
  int grid_y = rezY / stride;
